...
if docker ps -a --format '{{.Names}}' | grep -Eq "^${CONTAINER_NAME}$"; then
    ...
    echo "Creating new container: ${CONTAINER_NAME} with custom Postgres tuning..."
    docker run \
      --name "$CONTAINER_NAME" \
      --memory="$DB_MEM_LIMIT" \
      ...
      -c max_wal_size="$DB_MAX_WAL_SIZE" \
      -c maintenance_work_mem="$DB_MAINTENANCE_WORK_MEM" || { echo "Failed to start PostGIS container"; exit 1; }
fi
